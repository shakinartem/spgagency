      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
          <div className="panel-card dossier-card p-7 sm:p-8">
            <div className="grid gap-4 text-ink/90 sm:grid-cols-2">
              {[
                ...
                <motion.div
                  ...
                  className="rounded-3xl border border-ink/10 bg-white p-5 text-base leading-7 text-graphite/80"
                >
                  {item}
                </motion.div>
              ))}
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-[1.15fr_0.85fr]">
              ...
              <div className="rounded-[1.8rem] border border-ink/10 bg-white p-5">
                <p className="text-xs uppercase tracking-[0.28em] text-graphite/50">Формат</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {["контент", "сайт", "карты", "воронка", "аналитика", "репутация"].map((item) => (
                    <span key={item} className="rounded-full border border-ink/10 bg-white px-3 py-2 text-xs uppercase tracking-[0.18em] text-graphite/70">
                      {item}
                    </span>
                  ))}
                </div>
            ...
            <div className="mt-5 rounded-[1.8rem] border border-ink/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-graphite/50">Стоматология в фокусе</p>
                  <p className="mt-2 text-sm leading-6 text-graphite/80">Отдельно усиливаем проекты, где важны карты, локальный поиск, VK, доверие к врачам и понятный путь пациента до записи.</p>
                </div>
                <div className="agent-marker hidden text-xs uppercase tracking-[0.24em] text-graphite/70 sm:block">dental desk</div>
              </div>
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {dentalClients.map((client, index) => (
                  <motion.div
                    ...
                    className="rounded-[1.4rem] border border-ink/10 bg-white p-4"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-display text-2xl text-ink">{client.name}</p>
                        <p className="mt-1 text-sm uppercase tracking-[0.22em] text-graphite/60">{client.city}</p>
                      </div>
                      <span className="rounded-full border border-ember/25 bg-ember/10 px-3 py-1 text-[0.65rem] uppercase tracking-[0.24em] text-graphite/70">стоматология</span>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {client.channels.map((channel) => (
                        <span key={channel} className="rounded-full border border-ink/10 bg-white px-3 py-2 text-xs uppercase tracking-[0.16em] text-graphite/70">
                          {channel}
                        </span>
                      ))}
                    </div>
          ...
          <div className="grid gap-4">
            {stats.map((stat, index) => (
              ...
              >
                <p className="text-xs uppercase tracking-[0.28em] text-graphite/60">{stat.title}</p>
                ...
                <p className="mt-4 text-sm leading-6 text-graphite/80">{stat.note}</p>
              </motion.div>
            ))}
          </div>
        </div>
