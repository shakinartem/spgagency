    ...
    <section id="cases" className="section-shell px-4">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          ...
          description="Все кейсы хранятся в статическом файле и показываются здесь с фильтрацией по нишам. На карточке — короткая выжимка, внутри — полная задача, решение, цифры и артефакты."
        />
            ...
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm transition ${
                activeCategory === category
                  ? "border-ember/40 bg-ember/12 text-paper"
                  : "border-white/10 bg-white/5 text-sand/70 hover:text-paper"
              }`}
            >
              {category}
            </button>
          ...
          <AnimatePresence mode="popLayout">
            {filteredCases.map((item) => (
              <motion.article
                ...
                className="group panel-card flex h-full flex-col p-6"
              >
