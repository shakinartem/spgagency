...
function CaseLogo({ item }: { item: CaseStudy }) {
  ...
  const src = item.logoPath ?? `${basePath}assets/cases/${item.id}/logo.svg`;

  if (!visible) return <div className="logo-badge">{item.badge}</div>;

  return (
    <div className="flex h-14 min-w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/5 px-3">
      <img src={src} alt={item.name} className="h-9 w-auto object-contain" onError={() => setVisible(false)} />
    </div>
  );
}
...
export function Cases({ items }: CasesProps) {
  ...
  const filteredCases = useMemo(() => (activeCategory === "Все" ? items : items.filter((item) => item.category === activeCategory)), [activeCategory, items]);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 flex flex-wrap items-center gap-3">
          ...
          {caseCategories.map((category) => (
            <button key={category} type="button" onClick={() => setActiveCategory(category)} className={`rounded-full border px-4 py-2 text-sm transition ${activeCategory === category ? 'border-ember/40 bg-ember/12 text-paper' : 'border-white/10 bg-white/5 text-sand/70 hover:text-paper'}`}>
              {category}
            </button>
          ))}
        </div>
        <motion.div layout className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <AnimatePresence mode="popLayout">
            {filteredCases.map((item) => (
              <motion.article key={item.id} layout initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.28 }} className="group panel-card relative flex h-full flex-col p-6">
                <div className="flex items-start justify-between gap-4">
                  <CaseLogo item={item} />
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[0.65rem] uppercase tracking-[0.24em] text-sand/65">{item.category}</span>
                </div>
                <div className="mt-6">
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {item.metrics.slice(0, 4).map((metric) => (
                    <div key={metric.label} className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="text-[0.65rem] uppercase tracking-[0.24em] text-sand/55">{metric.label}</p><p className="mt-2 text-lg text-paper">{metric.value}</p></div>
                  ))}
                </div>
