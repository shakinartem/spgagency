  ...
  return (
    <>
      <form id="audit-form" onSubmit={handleSubmit} className="panel-card relative overflow-hidden p-6 sm:p-7">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(185,8,8,0.12),transparent_34%)]" />
        <div className="relative">
          <p className="text-xs uppercase tracking-[0.28em] text-sand/50">Форма аудита</p>
          ...
          <div className="mt-6 grid gap-4">
            <label className="grid gap-2">
              ...
              <input value={form.name} onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))} className="rounded-2xl border border-paper/10 bg-paper/[0.07] px-4 py-3 text-paper outline-none transition focus:border-ember/40" placeholder="Как к вам обращаться" />
            </label>
            <label className="grid gap-2">
              <span className="text-sm uppercase tracking-[0.2em] text-sand/60">Ниша</span>
              <input value={form.niche} onChange={(event) => setForm((prev) => ({ ...prev, niche: event.target.value }))} className="rounded-2xl border border-paper/10 bg-paper/[0.07] px-4 py-3 text-paper outline-none transition focus:border-ember/40" placeholder="Стоматология, медицина, B2B, недвижимость..." />
            </label>
            <label className="grid gap-2">
              <span className="text-sm uppercase tracking-[0.2em] text-sand/60">Контакт</span>
              <input value={form.contact} onChange={(event) => setForm((prev) => ({ ...prev, contact: event.target.value }))} className="rounded-2xl border border-paper/10 bg-paper/[0.07] px-4 py-3 text-paper outline-none transition focus:border-ember/40" placeholder="@username, телефон или ссылка" />
            </label>
            <label className="grid gap-2">
              <span className="text-sm uppercase tracking-[0.2em] text-sand/60">Что нужно усилить</span>
              <textarea value={form.task} onChange={(event) => setForm((prev) => ({ ...prev, task: event.target.value }))} className="min-h-[132px] rounded-2xl border border-paper/10 bg-paper/[0.07] px-4 py-3 text-paper outline-none transition focus:border-ember/40" placeholder="Кратко опишите задачу: поток заявок, сайт, репутация, контент, упаковка..." />
            </label>
          </div>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-white/90">Мы отправили заявку в Telegram. Сейчас откроется чат, чтобы клиент мог написать сразу.</p>
              </div>
            </div>
          </motion.div>
        ) : null}
