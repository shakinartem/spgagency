          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <h3 className="max-w-[14rem] font-display text-[2rem] leading-[0.95] text-ink">{item.title}</h3>
                <span className="shrink-0 rounded-full border border-ink/10 bg-white px-3 py-1 text-[0.58rem] uppercase tracking-[0.22em] text-graphite/70">
                  {item.note}
                </span>
              </div>
              ...
              <div className="mt-5 grid gap-4">
                <div>
                  <p className="text-[0.58rem] uppercase tracking-[0.26em] text-graphite/50">Типовая задача</p>
                  <p className="mt-2 text-sm leading-6 text-graphite/80">{item.problem}</p>
                </div>
                <div>
                  <p className="text-[0.58rem] uppercase tracking-[0.26em] text-graphite/50">Типовой результат</p>
                  <p className="mt-2 text-sm leading-6 text-ink/90">{item.result}</p>
                </div>
              </div>
            </motion.article>
          ))}
